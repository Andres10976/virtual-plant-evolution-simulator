...
import { fetchPlants as fetchPlantsApi, createPlant as createPlantApi, growPlant as growPlantApi, breedPlants as breedPlantsApi } from '../api/api';
...
export const growPlant = createAsyncThunk('plants/growPlant', async (plantId) => {
  const response = await growPlantApi(plantId);
  return response;
});

export const breedPlants = createAsyncThunk('plants/breedPlants', async ({plant1Id, plant2Id}) => {
...
export const plantsSlice = createSlice({
  ...
  reducers: {
    selectPlant: (state, action) => {
      state.selectedPlant = state.plants.find(plant => plant._id === action.payload);
    },
    deselectPlant: (state) => {
      state.selectedPlant = null;
    },
    updateEnvironment: (state, action) => {
      state.environmentalFactors = { ...state.environmentalFactors, ...action.payload };
    },
    ...
    updatePlant: (state, action) => {
      ...
      if (state.selectedPlant && state.selectedPlant._id === action.payload._id) {
...
export const { selectPlant, deselectPlant, updateEnvironment, resetEnvironment, clearError, updatePlant } = plantsSlice.actions;

// Selectors
export const selectAllPlants = (state) => state.plants.plants;
export const selectSelectedPlant = (state) => state.plants.selectedPlant;
export const selectEnvironmentalFactors = (state) => state.plants.environmentalFactors;
